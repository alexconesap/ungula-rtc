...
namespace ungula::rtc::detail {
    ...
    constexpr uint8_t daysInMonth(uint16_t y, uint8_t m) {
        constexpr uint8_t LUT[12] = {31, 28, 31, 30, 31, 30,
                                     31, 31, 30, 31, 30, 31};
        if (m < 1U || m > 12U) {
            return 0U;
        }
    ...
    inline int64_t toEpochMs(const DateTime& dt) {
        if (dt.year < 1970U || dt.month < 1U || dt.month > 12U ||
            dt.day < 1U || dt.day > daysInMonth(dt.year, dt.month) ||
        ...
        const int64_t seconds = static_cast<int64_t>(days) * 86400 +
                                static_cast<int64_t>(dt.hour) * 3600 +
                                static_cast<int64_t>(dt.minute) * 60 +
