...
namespace ungula::rtc::detail
{
    ...
    inline int64_t toEpochMs(const DateTime &dt)
    {
        if (dt.year < 1970U || dt.month < 1U || dt.month > 12U || dt.day < 1U ||
            dt.day > daysInMonth(dt.year, dt.month) || dt.hour > 23U || dt.minute > 59U || dt.second > 59U) {
            return 0;
        }
        ...
        const int64_t seconds = static_cast<int64_t>(days) * 86400 + static_cast<int64_t>(dt.hour) * 3600 +
